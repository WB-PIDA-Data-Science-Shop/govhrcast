...
s3_results <- rbindlist(list(
  run_s3(bra_ct_panel,           salary_scale_est, "3% uniform COLA",                   cola = 0.03),
  run_s3(bra_ct_panel,           salary_scale_est, "10% across-the-board",               cola = 0.10),
  run_s3(bra_ct_selective_panel, salary_scale_est, "selective: lower +8%, upper +2%",    cola = 0.03)
))
...
REF_DATE <- as.Date("2015-09-01")   # project from 2016 onwards

# Working contracts only
ct_base <- bra_hrmis_contract[
  ref_date == REF_DATE & contract_type_code %in% c("perm", "fterm", "temp")
]
pt_raw  <- bra_hrmis_personnel[ref_date == REF_DATE & status == "active"]
...
# ─────────────────────────────────────────────────────────────────────────────
# SECTION 1: Baseline — status quo projection (10 years, 3 % COLA, natural attrition)
# ─────────────────────────────────────────────────────────────────────────────
# Captures: wage bill trajectory with natural retirements, no active hiring,
# 3 % annual COLA.  Benchmark against which all other scenarios are compared.

cat("\n====  SECTION 1: Baseline — natural attrition + 3 % COLA  ====\n")
...
# Scenario 2a: Hiring freeze (no replacements, no COLA)
# Scenario 2b: Hiring freeze (no replacements, 3 % COLA)
# Scenario 2c: Full backfill (1:1 replacement of every retiree, 3 % COLA)
# Scenario 2d: Mass recruitment (2× replacement rate, 3 % COLA)
...
run_s2 <- function(hire_pol, label, cola = 0.03) {
  # NOTE: salary-scale warnings for new hire contracts are expected here.
  # The teaching dataset (bra_hrmis_*) uses est_id as the salary-scale key, but
  # new hire contracts generated in flow mode occasionally carry est_id values
  # that don't match the scale when replacement_rate > 1.  Those hires receive
  # NA salary (excluded from wage-bill sums via na.rm = TRUE).  Suppress to
  # keep output readable; replace with real salary-scale data for Zambia.
  res <- suppressWarnings(simulate_horizon(
    contract_dt        = data.table::copy(ct_base),
    personnel_dt       = data.table::copy(pt_base),
    salary_scale_dt    = data.table::copy(salary_scale_est),
...
cat("\n10-year results by recruitment scenario (Year 10, i.e. 2025):\n")
s2_results[year == 2025, .(
  scenario,
  n_headcount_end,
  n_hires,
  wage_bill_end              = round(wage_bill_end),
...
# Selective uplift: lower grades (A, B, C) get 8 %, upper grades get 2 %
# Apply as a one-off to ct_base salaries at period 0 (shift the starting state)
salary_scale_selective <- data.table::copy(salary_scale_grade)
salary_scale_selective[paygrade %in% c("A", "B", "C"),
                       gross_salary_lcu := gross_salary_lcu * 1.08]
salary_scale_selective[!paygrade %in% c("A", "B", "C"),
...
run_s3 <- function(ct, ss, label, cola = 0) {
  res <- simulate_horizon(
...
s3_results <- rbindlist(list(
  run_s3(ct_base,      salary_scale_est, "3% uniform COLA",                   cola = 0.03),
  run_s3(ct_base,      salary_scale_est, "10% across-the-board",               cola = 0.10),
  run_s3(ct_selective, salary_scale_est, "selective: lower +8%, upper +2%",    cola = 0.03)
))

cat("\nYear-1 and Year-10 wage bill by salary scenario:\n")
s3_results[year %in% c(2016, 2025), .(
  scenario,
  year,
  wage_bill_start            = round(wage_bill_start),
  wage_bill_end              = round(wage_bill_end),
...
# SECTION 4: Hiring freeze — detailed decomposition (5 years, no COLA)
# ─────────────────────────────────────────────────────────────────────────────
# Shows fiscal space created by a freeze and headcount implications.
# Cumulative exit_savings measures the total wages no longer paid out.
...
# SECTION 5: Promotion policy / regrading — by establishment × paygrade
# ─────────────────────────────────────────────────────────────────────────────
# Scenario 5a: No promotions (policy = NULL)
# Scenario 5b: Promotion freeze (multiplier = 0)
# Scenario 5c: Normal promotion pace (multiplier = 1.0)
# Scenario 5d: Accelerated regrading exercise (multiplier = 3.0)
#
# NOTE: promotion_effect = 0 is expected here with the BRA teaching data when
# only a single snapshot is available — simulate_promotions_transfers() needs
# >= 2 panel snapshots to build a movement baseline.  For Zambia live data,
# supply the two most recent payroll snapshots and the model auto-detects the
# baseline.
...
cat("\nYear-5 wage bill by promotion scenario:\n")
cat("(promotion_effect = 0 expected: single snapshot, no movement baseline)\n")
s5_results[year == 2020, .(
  scenario,
  n_promotions,
  n_transfers,
  wage_bill_end                     = round(wage_bill_end),
...
# SECTION 6: Establishment size reform — stock-based headcount targets
# ─────────────────────────────────────────────────────────────────────────────
# Zambia often needs to redistribute capacity across ministries/provinces.
# Scenario 6a: Status quo (no stock targets)
# Scenario 6b: 10 % workforce reduction across all establishments
# Scenario 6c: 20 % expansion in the 5 largest establishments
...
s6_results <- rbindlist(list(
  run_s6(NULL,                                        "status quo"),
  run_s6(make_stock_hire_policy(stock_targets_reduce), "10% reduction"),
  run_s6(make_stock_hire_policy(stock_targets_expand), "20% expansion (top 5 ests)")
))

cat("\nYear-5 wage bill by establishment size scenario:\n")
s6_results[year == 2020, .(
  scenario,
  n_headcount_end,
  n_hires,
  wage_bill_end                  = round(wage_bill_end),
...
# SECTION 7: Allowance restructuring
# ─────────────────────────────────────────────────────────────────────────────
# In Zambia, allowances (housing, transport, medical) often exceed base salary.
# Scenario 7a: Current structure (baseline)
# Scenario 7b: Cap allowances at 30 % of base salary (reform)
# Scenario 7c: Eliminate all allowances (consolidate into base pay)
...
run_s7 <- function(ct, label) {
  ss <- ct[, .(gross_salary_lcu = median(gross_salary_lcu, na.rm = TRUE)), keyby = est_id]
  res <- simulate_horizon(
    contract_dt        = data.table::copy(ct),
    personnel_dt       = data.table::copy(pt_base),
    salary_scale_dt    = data.table::copy(ss),
...
s7_results <- rbindlist(list(
  run_s7(ct_base,         "current allowances"),
  run_s7(ct_allow_capped, "capped at 30% of base"),
  run_s7(ct_no_allow,     "allowances eliminated")
))

cat("\nYear-1 and Year-5 wage bill by allowance scenario:\n")
s7_results[year %in% c(2016, 2020), .(
  scenario,
  year,
  wage_bill_start  = round(wage_bill_start),
  wage_bill_end    = round(wage_bill_end),
...
# SECTION 8: Combined policy scenario — "fiscal reform package"
# ─────────────────────────────────────────────────────────────────────────────
# Reform:     early retirement (58), full backfill, 5 % COLA, allowance cap
# Status quo: retirement at 60, hiring freeze, 3 % COLA, current allowances
...
res_reform <- suppressWarnings(simulate_horizon(
  contract_dt        = data.table::copy(ct_allow_capped),
  personnel_dt       = data.table::copy(pt_base),
  salary_scale_dt    = data.table::copy(ss_reform),
  n_periods          = 10L,
  retirement_policy  = ret_policy_early,
...
comparison <- rbind(
  res_reform$summary_dt[,
    scenario := "reform (early retire + backfill + 5% COLA + allow cap)"],
  res_status_quo$summary_dt[,
    scenario := "status quo (retire 60 + freeze + 3% COLA)"]
)
...
# SECTION 9: Horizon sensitivity — 5, 10, 20 years
# ─────────────────────────────────────────────────────────────────────────────
# Status-quo scenario projected across three planning horizons.
...
# ─────────────────────────────────────────────────────────────────────────────
# SECTION 10: generate_scenario_matrix() — full policy grid for dashboard
# ─────────────────────────────────────────────────────────────────────────────
#
# DESIGN PRINCIPLES
# -----------------
#  - All three policy modules (retirement, movement, hiring) are always active.
#    No NULL policies — every scenario uses status-quo defaults for levers not
#    under variation.
#  - Full panel (bra_ct_panel / bra_pt_panel) is passed so that:
#      (a) movement baseline estimation uses all 12 annual snapshots, and
#      (b) status_quo hiring rate is estimated from historical flow data.
#  - Movement policy pre-sets salary_scale_est_grade (est_id × paygrade);
#    since simulate_horizon now only injects salary_scale_dt when the policy
#    has no pre-set scale, this is not overwritten.
#
# GRID: 3 × 3 × 4 × 3 = 108 scenarios × 10 periods = 1 080 rows
#
#   salary_growth_rate      : 0 %, 3 %, 8 %          (no COLA / moderate / high)
#   retirement_min_age      : 55, 60, 65              (early / standard / late)
#   hiring_replacement_rate : 0, 0.5, 1.0, 2.0       (freeze / partial / full / expansion)
#   movement_promotion_mult : 0, 1.0, 3.0             (freeze / normal / regrading)
#
# ZAMBIA POLICY INTERESTS COVERED
# --------------------------------
#  1. Wage bill horizons          → salary_growth_rate lever
#  2. Grade-stratified analysis   → movement group_cols = c("est_id", "paygrade")
#  3. Recruitment variations      → hiring_replacement_rate (0 = freeze → 2 = expansion)
#  4. Salary increases            → salary_growth_rate lever
#  5. Allowance restructuring     → initial salary_scale reflects allowance policy
#  6. Hiring freezes              → hiring_replacement_rate = 0
#  7. Promotion / regrading       → movement_promotion_multiplier lever
#  8. Establishment size reform   → captured via hiring_replacement_rate > 1
#
# ─────────────────────────────────────────────────────────────────────────────

cat("\n====  SECTION 10: Full scenario matrix (108 scenarios × 10 periods)  ====\n")

devtools::load_all(quiet = TRUE)   # ensure latest compiled code is active

# ── Base policies (status-quo defaults for each module) ────────────────────
...
# Movement: est_id × paygrade — salary_scale pre-set so simulate_horizon
# does NOT overwrite it with the est_id-only salary_scale_est.
move_policy_sq <- list(
  group_cols           = c("est_id", "paygrade"),
  salary_scale         = data.table::copy(salary_scale_est_grade),
...
# Hiring: status_quo replicates historical rates from the panel.
# salary_scale not pre-set here — simulate_horizon injects salary_scale_est.
hire_policy_sq <- list(
  mode       = "status_quo",
  group_cols = "est_id",
  rate_mult  = 1.0
...
param_grid <- list(
  salary_growth_rate            = c(0.00, 0.03, 0.08),
  retirement_min_age            = c(55L, 60L, 65L),
  hiring_replacement_rate       = c(0.0, 0.5, 1.0, 2.0),
  movement_promotion_multiplier = c(0.0, 1.0, 3.0)
)

# ── Run the scenario matrix ─────────────────────────────────────────────────

scenario_matrix <- suppressWarnings(
  generate_scenario_matrix(
    ...
    param_grid         = param_grid,
    n_periods          = 10L,
    retirement_policy  = ret_policy_sq,
    movement_policy    = move_policy_sq,
...
cat(sprintf(
  "Matrix complete: %d scenarios × %d periods = %d rows\n",
  data.table::uniqueN(scenario_matrix$scenario_id),
  data.table::uniqueN(scenario_matrix$period_date),
  nrow(scenario_matrix)
))

# ── Quick diagnostic: confirm promotions are firing ─────────────────────────
cat("\nPromotion activity (non-zero scenarios only):\n")
promo_check <- scenario_matrix[
  n_promotions > 0,
  .(
    n_scenarios_with_promotions = data.table::uniqueN(scenario_id),
    mean_n_promotions           = round(mean(n_promotions), 1),
    max_n_promotions            = max(n_promotions),
    mean_promo_effect           = round(mean(promotion_effect))
  )
]
promo_check

# ── Summary: final-period wage bill across all scenarios ────────────────────
cat("\nFinal-period wage bill summary (period 10):\n")
scenario_matrix[
  period_date == max(period_date),
  .(
    n_scenarios     = .N,
    wb_min          = round(min(wage_bill_end)),
    wb_p25          = round(quantile(wage_bill_end, 0.25)),
    wb_median       = round(median(wage_bill_end)),
    wb_p75          = round(quantile(wage_bill_end, 0.75)),
    wb_max          = round(max(wage_bill_end))
  )
]

# ── Top / bottom 5 scenarios by final wage bill ─────────────────────────────
final_period <- scenario_matrix[period_date == max(period_date)]

cat("\nTop 5 highest wage bill at period 10:\n")
final_period[
  order(-wage_bill_end)
][1:5, .(
  scenario_label,
...
cat("\nTop 5 lowest wage bill at period 10:\n")
final_period[
  order(wage_bill_end)
][1:5, .(
  scenario_label,
